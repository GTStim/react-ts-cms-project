...
const NavigatorPortrait: React.FC<{ routes: RouteType[] }> = ({ routes }) => {
    ...
    function getListItems(): React.ReactNode {
        return routes.map((i) => (
            <ListItem onClick={toggleOpen} component={Link} to={i.to} key={i.to}>
                {i.label}
            </ListItem>
        ));
    }
    return (
        <Box sx={{ marginTop: { xs: '15vh', sm: '20vh' } }}>
            <AppBar position="fixed" sx={{ backgroundColor: 'powderblue' }}>
                <Toolbar>
                    <IconButton onClick={toggleOpen} sx={{ color: 'black' }}>
                        <Menu />
                    </IconButton>

                    <Typography
                        sx={{ width: '100%', textAlign: 'center', fontSize: '1.5em', color: 'black' }}
                    >
                        {getTitle()}
                    </Typography>
